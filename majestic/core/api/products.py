"""Desktop API — Solo Product Forge reports.

  GET  /products/{profile}          list saved reports (date + summary)
  GET  /products/{profile}/{date}   full report (markdown + parsed items)
  POST /products/{profile}/run      generate now (returns the report)

Reports are stored under ``profiles/<name>/workspace/products/<date>.{md,json}``.
"""
...
def _products_dir(profile: str) -> Path:
    return _PROFILES_ROOT / profile / "workspace" / "products"
    ...
    if d.exists():
        for f in sorted(d.glob("*.json"), reverse=True):
            ...
            reports.append({
                "date":  f.stem,
                "count": len(items),
                "top":   [it.get("name", "") for it in items[:3] if isinstance(it, dict)],
            })
